...
class GlobalTracker(object):
    def __init__(self,MAX_TRACKERS=15):
        self.det = Detector()
    ...
    def pipeline(self, boxes,scores,classes,img,score_thresh, iou_threshold = 0.3):
        ...
        # z_box -> bounding box coordinates
        # idx_vec -> index of the subset of classes bounding boxes are placed upon.
        ...
        for trk in self.tracker_list:
            if ((trk.hits >= self.min_hits) and (trk.no_losses <=self.max_age)):
                ...
                # TODO: REMOVE IN FUTURE REVISION
                img= helpers.draw_box_label(img, bounding_boxes) # Draw the bounding boxes on the
